...
class load_file extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Future<TransferCat> loadAsterix(Uint8List fileBytes) {
      List<CAT10> cat10All = [];
      List<CAT21> cat21All = [];
      List<CATALL> catAll = [];
      ...
      for (int k = 0; k < messages.length; k++) {
        ...
        if (messages[k][0] == 10) {
          ...
          cat10All.add(cat10);
          catAll.add(new CATALL.fromCat10(cat10, _catProvider.first_time));
        } else if (messages[k][0] == 21) {
          cat21 = new CAT21(
              helpDecode, cat21helper, messages[k], k, messagesBinary[k]);
          cat21All.add(cat21);
          catAll.add(new CATALL.fromCat21(cat21, _catProvider.first_time));
        }
      }

      EasyLoading.dismiss(animation: true);
      return Future.value(new TransferCat.fromCats(cat10All, cat21All, catAll));
    }

    loadIsolate() {
      FilePickerCross.importFromStorage(fileExtension: 'ast').then((myFile) {
        ...
        if (myFile != null) {
          EasyLoading.show(status: 'loading...').then((value) async {
            ...
            // Chosen
            fileBytes =; */
            compute(loadAsterix, myFile.toUint8List()).then((transferCat) {
