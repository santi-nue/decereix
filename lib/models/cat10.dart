...
//CALCULATED ACCELERATION
  ...
  CAT10(HelpDecode decode,CAT10Helper cat10Helper,Uint8List message, int id, List<String> messageBinary){
    try{
      ...
      if (FSPEC[2] == '1')
      {
        index = cat10Helper.Compute_Target_Report_Descriptor(messageBinary, index);
      ...
      if (FSPEC[4] == '1')
      {
        index = cat10Helper.Compute_Position_in_WGS_84_Coordinates(messageBinary, index);
        this.LatitudeWGS_84 = cat10Helper.Latitude_in_WGS_84;
        this.LatitudeWGS_84 = cat10Helper.Longitude_in_WGS_84;
      }
      if (FSPEC[5] == '1')
      {
        index = cat10Helper.Compute_Measured_Position_in_Polar_Coordinates(messageBinary, index);
      ...
      if (FSPEC[6] == '1')
      {
        index = cat10Helper.Compute_Position_in_Cartesian_Coordinates(messageBinary, index);
      ...
      if (FSPEC.length > 8)
      {
        if (FSPEC[7] == '1')
        {
          index = cat10Helper.Compute_Track_Velocity_in_Polar_Coordinates(messageBinary, index);
        ...
        if (FSPEC[8] == '1')
        {
          index = cat10Helper.Compute_Track_Velocity_in_Cartesian_Coordinates(messageBinary, index);
        ...
        if (FSPEC[11] == '1')
        {
          index = cat10Helper.Compute_Mode_3A_Code_in_Octal_Representation(messageBinary, index);
        ...
        if (FSPEC[13] == '1')
        {
          index = cat10Helper.Compute_Target_Identification(messageBinary, index);
